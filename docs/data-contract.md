...
API・保存形式の契約です。文書監査そのものは実装適合性の検証ではありません。実装後の結果は[実装検証記録](implementation-verification.md)、実行手順は[運用手順](study-deployment.md)を参照してください。
...
1. 回答確定時にブラウザー内へ保存してAPIへ送り、DB保存成功の確認後に次の場面へ進む。
...
未同意の作成、旧版の新規開始は拒否します。新規受付だけを停止する制御は実収集前へ延期し、公開中の合成データ用デモでは新規開始を受け付けます。
...
既存IDの開始再送は認証・期限の照合を行い、再作成・再抽選しません。期限判定はサーバー時刻が`resume_expires_at`未満なら有効、一致以降なら期限切れです。保持期限後に削除済みの場合は照合できず、期限切れ410の応答を保証しません。
...
同じ仕様の運用中は、このmanifestで元の群・順番・提示文を再現します。提示文・質問文の正本管理は行いません。セッションごとに保存された全文を分析時に文字列照合し、提示内容の一致・差異を確認します。版番号だけで内容の一致を判定しません。これは刺激を実行中に変更する機能を追加する意味ではありません。参加途中に質問文・study/schema/consent版を変更する運用は、今回明示的に想定しません。更新をまたぐ再開、複数schemaへの互換対応、データ移行は受け入れ対象外です。これらが必要になった場合は別途仕様を決定します。
...
各場面のDB保存成功を確認してから次へ進むため、未保存の確定回答は最大1場面です。複数場面をためる送信キューは設けません。最終回答の保存結果が完了を示すか、期限内の進捗照合で完了を確認できたら完了画面を表示します。
送信が失敗したら、その場面の確定回答を保持して停止します。手動再送で保存成功を確認してから次へ進みます。再送ボタンの連打でも同じ送信処理を複数起動しません。
通信障害中は後続場面への回答を続けられません。完全オフラインでの新規開始・再開も保証しません。
...
同じ公開元の進捗保存領域を使用する実行は1タブに限定します。受け入れ試験は、2つ目のタブが開始・進捗変更できず、元のタブを閉じた後に再開できる代表例にまとめます。ID発行前から進捗の消去・新規開始まで同じ排他範囲に含め、別IDによる共有保存キーの上書きも防ぎます。Web Locksによる排他を第一案とし、ロック取得できないタブには利用中の案内を表示します。
...
バックアップを復元した場合は受付再開前に期限切れ削除を再実行します。これは復元時の運用上の注意であり、復元シナリオの実装・試験を今回の完成条件には含めません。
...
スクリプトは`npm run data:export -- --target local|demo --output <file>`として実装済みです。`demo`は合成データ用の公開環境1つを指します。Devbox経由の具体例と出力名は[運用手順](study-deployment.md#csv取得)を参照してください。

毎回、完了試行CSV・未完了試行CSV・全セッションCSV・抽出メタデータの4ファイルを固定出力します。完了／未完了の抽出オプションは設けません。対象が0件のCSVもヘッダー付きで出力します。対象環境と出力先の指定は残します。
完了試行CSVは保持期限内の完了セッションの回答、未完了試行CSVは保持期限内の未完了セッションの保存済み回答を含みます。
セッションCSVは、保持期限内の全セッションを1行1セッションで出力します。0回答も含め、参加ID、study/consent/schema版、割り当て群、開始・再開期限・削除期限・完了時刻、保存済み試行数、状態を出力します。状態は完了、期限内未完了、期限切れ未完了を区別します。
3種のCSVはUTF-8とし、取得時刻・対象環境・抽出条件・schema版・各件数を記録したメタデータファイルを出力します。セッションCSVの未完了者を主要分析に混ぜません。
完了・未完了の試行CSVは共通の列構成とし、列は参加ID、状態、study/consent/schema版、割り当て群（`assigned_condition`）、場面、試行条件（群と一致）、順序、2評価、所要時間、区間・再開回数・再提示情報、各時刻、提示文・質問文です。分析時の文字列照合に必要な全文をセッションのmanifestから出力し、CSVとして引用符・改行をエスケープします。
再開トークン・ハッシュ、内部設定、個人の連絡先は出力しません。
今回の受け入れ試験は回答追加のない少数の固定した合成データで実施し、取得件数とDB件数を照合します。完了、途中、0回答、期限切れを含め、取得開始時刻を保持期限の抽出基準とします。500件規模の取得確認や、その規模で結果上限に対応するページングの必要性の確認は将来の運用前に行います。募集と同時進行の厳密なスナップショット取得は保証対象外です。
