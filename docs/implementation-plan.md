...
- 約5分の日本語ウェブ実験。説明・同意、8場面の課題、保存確認、簡素な完了画面。
...
| 区分 | 内容 |
| --- | --- |
| ユーザーが決定 | 題材、理解を示す一言の比較、参加者内、固定文章、指定された希望、約5分、2尺度、PC向け、逐次送信、Workers+D1、TypeScript、ランダムID、CSV、24時間再開、30日保持 |
| 本計画で具体化した案 | 8場面、2つの相補的な割り当て、7段階の質問文、APIとDB、開始時manifest、ローカル送信待ち、再開トークン、毎時削除、フォルダーと段階分け |
...
- 8場面の状況、2種類の前置き、共通の提案本文を作成する。
- 提案が希望に適合するか、前置きの長さ・丁寧さ・具体性が不用意に偏らないか確認する。
- 2尺度、L0/L1の条件対応、ランダム順、study/schema/consent版を定義する。
...
- 各参加者の8場面に重複がなく、両条件4場面ずつになる。
- L0/L1間で各場面の条件が反転し、本文は場面内で一致する。
- 順列はmanifestとして保存・再利用でき、再開時に再抽選されない。
...
- 7場面では完了せず、8場面保存後に完了し、完了の再送も成功する。
...
- 500セッション・4,000試行の合成データを欠落なく出力できる。
