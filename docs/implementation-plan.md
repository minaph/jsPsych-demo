...
- 場面ごとの逐次保存、通信失敗時の手動再送、同じブラウザーから24時間以内の再開。
- デモデータの30日保持と期限切れ削除。
...
| 監査で追加決定 | 4回答保存で自動完了、手動再送のみ・15秒待機、0回答を含むセッションCSV、課題再開と保存確認の区別、Chromeのみ、500完了セッションの整合性、途中更新は想定外、公開後の動作まで完成条件 |
...
仮の研究者情報を実情報のように公開せず、実参加者用の受付を停止したまま、別のローカル・検証用環境で合成データの受付を有効にして画面と保存を検証します。停止中の公開環境のAPIを迂回する仕組みは設けません。
...
- PC向けの対応方針を決める。Chrome安定版のみを動作保証対象とし、OS・版を記録する。幅1024 CSS px以上とキーボード操作を確認し、対象幅・必須機能を満たさない場合は開始前に案内する。
...
- 提案が希望に適合するか確認する。4場面の全文を固定し、場面内の提案本文一致、前置きの文数・敬体統一、文字数差の記録を比較表で確認する。文字数差の上限や操作の効果の証明は合格条件にしない。
...
- 別タブは同じ公開元の進捗を同時に変更できない。ID発行前の開始競合、進捗消去も排他対象とする。
...
- 試行CSVは既定の完了データと、明示指定した途中データを分ける。別のセッションCSVには保持期限内の全セッションを0回答も含めて出力する。
...
- 固定した500完了セッション・2,000試行を欠落なく出力できる。別途0回答・1〜3回答・期限切れ・保持期限切れの合成データを用意し、両CSVの対象と件数をDBと照合する。同時500名や応答速度の保証は含めない。
- 未完了、保持期限切れ、別環境の回答が既定の試行CSVに混ざらない。24時間経過後も30日の保持期限内であれば、完了セッションは取得する。
- トークンやハッシュをCSVに出さない。
...
| データ | 0回答を含むセッションCSV、未完了の分離、500件規模の取得、token非出力、環境分離 |
