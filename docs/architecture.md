...
実装後のファイル構成・設定値・生成物とコマンドは[実験デモの運用手順](study-deployment.md)に記録しています。以下の設計上の配置案と、現在の実ファイル名は区別してください。
...
`vite preview`などのローカル確認が通っても、公開環境でのAPI・bindingの正常動作とCron設定は別に確認します。公開Cronの実行成功確認は運用確認とし、完成前に実行を待つことは必須にしません。
...
Reactは説明・同意・再開確認・保存状況・完了画面を担当します。対応保証は検証時のChrome安定版とし、通常のPC幅でキーボードのみでも全操作を完遂できることを確認します。OS・版・検証した幅を一括した確認記録に残します。固定幅による開始禁止は設けず、小画面にはPC利用を案内します。Web LocksやlocalStorageなどの必須機能不足は開始前に案内します。Edge、Firefox、Safariは今回の動作保証対象外です。
...
試行の完了通知では、プラグイン固有の生データから必要項目を取り出し、ブラウザー保存と送信処理へ渡します。各場面のDB保存成功後に次へ進むため、送信待ちは最大1場面とし、複数場面のキューは設けません。
...
src/
  ...
  persistence/       # localStorage、1場面の送信待ち、再開、同時タブ制御
...
tests/               # API・DBの重要箇所のテスト。ブラウザー自動化は任意
docs/                # この計画と実装後の運用手順
```
...
| 設定 | 配置と扱い |
| --- | --- |
| `DB` binding | ローカルと合成データ用公開で別DBを指定。公開Worker/D1は1組で完成可 |
| `STUDY_VERSION` / `CONSENT_VERSION` | コードに固定。保存行にも記録 |
| `COLLECTION_ENABLED` | 実収集前に整備する受付制御。今回の合成データ用デモの完成条件から外す |
| `RESUME_TTL_HOURS=24` | サーバーを期限判定の基準とする |
| `DEMO_RETENTION_DAYS=30` | サーバー作成時刻から計算。最後の回答で延長しない |
| 責任者・連絡先・説明文 | 合成データ用であることを明示し、仮情報を実情報として表示しない。研究者情報の未設定時の受付制御は実収集前に整備する |
| Cloudflare管理認証 | 開発者の認証環境のみ。ブラウザー用環境変数へ置かない |

提示文・質問文は各セッションのmanifestに全文保存します。正本管理は行わず、分析時に保存された文字列を照合します。

今回の公開先は合成データの接続・操作確認用デモです。実参加者の回答を収集する環境の準備は今回の完成条件に含めません。収集開始前に研究準備と検証用データとの環境分離を行います。
...
今回の完成条件は、少数の代表的な合成データによる保存・取得の整合性確認です。100〜500名は運用想定であり、その規模の取得・性能は未検証です。500件規模の取得確認と運用規模に応じたCPU時間・読み書き量・失敗率の確認は、将来の運用前に行います。
